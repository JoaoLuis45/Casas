...
try
  if (StrToInt(txtPayday.Text) > 30) or (StrToInt(txtPayday.Text) < 1) then begin
       ...
       ShowMessage('Preencha todos os campos corretamente!');
        ...
        if Application.MessageBox('Deseja alugar esta casa?','Atenção',MB_ICONWARNING+MB_OKCANCEL) = mrOk then
          begin
...
begin
    ...
    lblType.Caption := 'Selecione pelo Número da casa:';
    DM.sqlFindHouse.Close;
...
begin
  case rgType.ItemIndex of
    0: lblType.Caption := 'Selecione pelo Número da casa:';
